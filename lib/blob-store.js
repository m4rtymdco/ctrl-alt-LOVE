...
async function writeManifest(tracks) {
  ...
  await put(MANIFEST, JSON.stringify({ tracks }), blobAuth({
    access: 'public',
...
async function addTrack({ title, artist, audioBuffer, audioMime, photoBuffer, photoMime }) {
  const id = Date.now();
  const audio = await put(`${PREFIX}audio-${id}`, audioBuffer, blobAuth({
    access: 'public',
  ...
  if (photoBuffer && photoBuffer.length) {
    const photo = await put(`${PREFIX}photo-${id}`, photoBuffer, blobAuth({
      access: 'public',
...
async function deleteTrack(id) {
  ...
  try {
    const { blobs } = await list(blobAuth({ prefix: `${PREFIX}audio-${id}`, limit: 10 }));
    const { blobs: photos } = await list(blobAuth({ prefix: `${PREFIX}photo-${id}`, limit: 10 }));
